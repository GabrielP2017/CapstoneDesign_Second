  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/tracking/${trackingNumber}`
      );
      const data = await response.json();

      if (response.ok) {
        // 검색 성공 시 TrackingStatus 페이지로 이동하거나 결과 표시
        console.log("조회 결과:", data);
        // TODO: 페이지 전환 또는 결과 표시 로직
      } else {
        console.error("조회 실패:", data.detail);
      }
    } catch (error) {
      ...
      <div className="w-full max-w-2xl px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-slate-600 dark:text-slate-400">
            운송장 번호를 입력하면 실시간으로 통관 상태를 확인할 수 있습니다
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="relative">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-14 pr-32 py-4 text-lg bg-white dark:bg-slate-800 border-2 
              border-slate-200 dark:border-slate-700 rounded-2xl text-slate-800
              dark:text-white placeholder-slate-400 focus:outline-none focus:border-blue-500
              focus:ring-4 focus:ring-blue-500/20 transition-all"
            />
            <button
              type="submit"
        ...
        <div className="mt-8 flex justify-center space-x-4">
          <button
            ...
            dark:text-slate-400 hover:text-blue-500 transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-2 px-4 py-2 text-sm text-amber-600 
            dark:text-amber-400 hover:text-amber-500 transition-colors disabled:opacity-50"
          >
